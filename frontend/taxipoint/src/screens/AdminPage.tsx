...
const AdminPage: React.FC<AdminPageProps> = ({ onLogout, user }) => {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 font-sans text-gray-200">
      <ToastContainer position="top-center" theme="dark" />

      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-blue-400 drop-shadow-lg">Admin Dashboard</h1>
          <button
        ...
        {/* Form */}
        <div className="bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-2xl mb-8 border border-gray-700">
        ...
        <div className="bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-2xl border border-gray-700">
          <h2 className="text-xl sm:text-2xl font-semibold text-blue-400 mb-4">Existing Taxi Ranks</h2>
            ...
            <table className="min-w-full divide-y divide-gray-700">
              <thead className="bg-gray-900 sticky top-0">
                <tr>
                  ...
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Address</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">District</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Routes</th>
                  <th className="px-4 sm:px-6 py-3 text-right text-xs font-medium text-gray-400 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="bg-gray-800 divide-y divide-gray-700">
                {taxiRanks.map((rank) => (
                  <tr key={rank.id} className="hover:bg-gray-700 transition-colors">
                    <td className="px-4 sm:px-6 py-4 text-sm font-medium break-words">{rank.name}</td>
                    <td className="px-4 sm:px-6 py-4 text-sm text-gray-300 break-words">{rank.address}</td>
                    <td className="px-4 sm:px-6 py-4 text-sm text-gray-300 break-words">{rank.district}</td>
                    <td className="px-4 sm:px-6 py-4 text-sm text-gray-300 break-words">{Array.isArray(rank.routesServed) ? rank.routesServed.join(", ") : ""}</td>
                    <td className="px-4 sm:px-6 py-4 text-right text-sm font-medium">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => handleEdit(rank)} className="text-indigo-400 hover:text-indigo-600 transition-colors p-1">
                          <MdEdit width={18} height={18} />
                        </button>
                        <button
                          onClick={() => {
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-900 p-6 sm:p-8 rounded-2xl shadow-2xl w-full max-w-sm transform scale-105">
            <h3 className="text-lg sm:text-xl font-bold text-red-400 mb-4">Confirm Deletion</h3>
