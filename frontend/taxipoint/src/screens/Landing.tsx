...
const Landing = ({ user }: LandingProps) => {
  ...
  // ✅ Fetch Taxi Ranks Nearby
  const fetchNearbyTaxiRanks = async (
    lat: number,
    lng: number,
    radius: number = 5000
  ) => {
  ...
  const searchTaxiRanks = async (query: string) => {
    ...
    try {
      const res = await fetch(
        `https://taxipoint-backend.onrender.com/api/taxi-ranks/search?query=${encodeURIComponent(
          query
        )}`,
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
  ...
  const fetchIncidents = async () => {
    try {
      const res = await fetch(
        "https://taxipoint-backend.onrender.com/api/incidents",
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
      const data = await res.json();
  ...
  const groupIncidentsByLocation = (incidents: Incident[]) => {
    ...
    incidents.forEach((incident) => {
      const key = `${incident.latitude.toFixed(precision)}-${incident.longitude.toFixed(
        precision
      )}`;
  ...
  // ✅ Initial Load: Taxi Ranks + Incidents
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => fetchNearbyTaxiRanks(pos.coords.latitude, pos.coords.longitude),
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
          ...
          >
            <Popup>
              <div className="space-y-1">
                ...
                {rank.description && (
                  <p>
                    <b>Description:</b> {rank.description}
                  </p>
                )}
                <p>
                  <b>Address:</b> {rank.address}
                </p>
                <p>
                  <b>District:</b> {rank.district}
                </p>
                {rank.routesServed.length > 0 && (
                  <p>
                    <b>Routes:</b> {rank.routesServed.join(", ")}
                  </p>
                )}
                {rank.phone && (
                  <p>
                    <b>Phone:</b> {rank.phone}
                  </p>
                )}
              </div>
            </Popup>
      ...
      {/* Welcome Banner */}
      <div className="absolute top-2 left-4 z-[999] pointer-events-none">
